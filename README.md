...
### **2. `/pp` - 스마트 프롬프트 추천기 v2.3.1**
AI가 사용자 프롬프트의 의도와 복잡성을 분석하여 최적의 변형을 동적으로 추천하는 지능형 커맨드입니다. 상황에 맞는 개수(2-5개)와 별점 평가를 제공하며, 접근법당 하나의 최적화된 프롬프트만 제시하여 사용성을 개선했습니다.
...
- 🎯 **단순화된 사용성**: 접근법당 하나의 최적화된 프롬프트만 제시 (v2.3.1 신규)
- 📝 **맥락 인식**: 프로젝트와 대화 기반 맥락 자동 반영 (v2.3.1 신규)
- 🎮 **상호작용 모드**: 인터랙티브한 프롬프트 생성 (v2.3.1 신규)
- 🌟 **템플릿 확장**: 시나리오/역할/도메인 특화 템플릿 (v2.3.1 신규)
...
```
/sc:enhance 맥락 인식과 템플릿 다양성을 통한 즉각적인 사용성 개선 --quick-wins --context-aware
```
```

### **상황별 추천 (v2.3 AI 기반 실용적 활용)**
...
# v2.3.1 빠른 테스트 (단순화된 프롬프트 적용)
/pp "웹사이트 디자인 해줘"             # AI가 4가지 맞춤 접근법 추천 (별점 포함, 각각 단일 프롬프트)
/pp "인증 버그 수정해줘"            # AI가 3가지 해결 방식 추천 (단순화된 사용성)
/pp "pp 명령어 개선 방안 제안해줘"      # AI가 3가지 개선 접근법 추천 (사용자 피드백 반영)
...
#### **1. v2.3.1 AI 기능 활용**
- **지능적 분석**: AI가 프롬프트 의도와 복잡성을 자동 분석
- **유연한 추천**: 상황에 맞는 최적의 개수(2-5개)와 순위 제공
- **별점 평가**: 각 추천의 효과성을 ⭐로 명확히 표시
- **특징 설명**: 각 추천의 강점과 적합한 상황 구체적으로 설명
- **추천 이유**: 왜 이 프롬프트가 효과적인지 이유 명시
- **단순화된 사용성**: 접근법당 하나의 최적화된 프롬프트만 제시 (v2.3.1 신규)

...
| `sc:architecture` | `sc:design` | 아키텍처 설계 기능 | 시스템 설계 |
| `sc:template` | `sc:design` | 템플릿 설계 기능 | 템플릿 생성 |
...
과거 오류:
→ /sc:architecture  # 존재하지 않는 명령어
→ "Unknown slash command: sc:architecture"

현재 처리:
→ 명령어 검증: sc:architecture 없음
→ 대체 명령어: sc:design
→ 안내 메시지: "sc:architecture 대신 sc:design를 사용합니다"
...
### 2단계: 유사성 기반 대체
```bash
# 유사도 기반 대체 (실제 검증된 명령어)
/sc:enhance → ❌ 없음 → sc:improve (유사도: 95%)
/sc:optimize → ❌ 없음 → sc:improve (유사도: 90%)
/sc:architecture → ❌ 없음 → sc:design (유사도: 95%)
/sc:template → ❌ 없음 → sc:design (유사도: 85%)
/sc:create → ❌ 없음 → sc:implement (유사도: 85%)
```

### 3단계: 기능별 그룹핑
```bash
# 기능 그룹별 대체 (실제 존재하는 명령어만)
분석 관련: sc:examine → sc:analyze
개발 관련: sc:build → sc:build (존재함)
설계 관련: sc:architect → sc:design
템플릿 관련: sc:template → sc:design
```
